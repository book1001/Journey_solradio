...
const ARENA_TOKEN = 'pXnEuRGCbxpvOyf9kf98rJ1cjLPi_4AjqljnYVRFXaw';
const CHANNEL_ID = 'fcumona-4ui';

app.post('/upload-to-arena', upload.single('image'), async (req, res) => {
  const imagePath = req.file.path;
  const form = new FormData();
  form.append('content[type]', 'Image');
  form.append('content[image]', fs.createReadStream(imagePath));
  form.append('content[connected_to_channel_slug]', CHANNEL_ID);

  const response = await fetch('https://api.are.na/v2/content', {
    method: 'POST',
    headers: {
